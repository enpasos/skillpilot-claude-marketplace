...
- **Current 1.1.5 replacement candidate:** install it only through an explicitly
  ...
  for 1.1.5 is still pending, and no earlier package is a supported fallback
...
1. On an eligible paid Claude account, add the authorized SkillPilot Marketplace
   ...
   **SkillPilot Coach v1** 1.1.5 candidate and enable it.
...
native Claude app on Android. Exact-candidate 1.1.5 acceptance must still
...
1.1.5 or settle whether Anthropic's official plugin distribution serves either
