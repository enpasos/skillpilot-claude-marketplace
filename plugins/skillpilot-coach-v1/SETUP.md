...
- **Current 1.1.4 replacement candidate:** install it only through an explicitly
  ...
  for 1.1.4 is still pending, and no earlier package is a supported fallback
  for candidate acceptance. Preparing this candidate does not update existing
  1.1.3 installations or the deployed download.
...
1. On an eligible paid Claude account, add the authorized SkillPilot Marketplace
   ...
   **SkillPilot Coach v1** 1.1.4 candidate and enable it.
...
native Claude app on Android. Exact-candidate 1.1.4 acceptance must still
...
1.1.4 or settle whether Anthropic's official plugin distribution serves either
