...
- **Current 1.1.8 replacement candidate:** install it only through an explicitly
  ...
  for 1.1.8 is still pending, and no earlier package is a supported fallback
...
## Current 1.1.8 candidate test

1. This 1.1.8 candidate is local and unpublished. During an explicitly authorized
   candidate test on an eligible paid Claude account, use the exact prepared
   **SkillPilot Coach v1** 1.1.8 package and enable it. The public Marketplace at
...
native Claude app on Android. Exact-candidate 1.1.8 acceptance must still
...
1.1.8 or settle whether Anthropic's official plugin distribution serves either
